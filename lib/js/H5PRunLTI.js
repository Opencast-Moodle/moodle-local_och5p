(function ($) {
    ...
    function runLti() {
        ...
        var ltiParams = opencastLtiParams.main;
        ...
        var searchNodeLtiParams = opencastLtiParams.search ? opencastLtiParams.search : null;
        if (searchNodeLtiParams) {
            performLti('searchNodeLtiLaunchForm', searchNodeLtiParams);
        }
    }

    function performLti(formName, ltiParams) {
        
        var endpoint = ltiParams.endpoint;
        delete ltiParams.endpoint;
        var ltiLaunchForm = H5P.jQuery('<form action="' + endpoint + '" id="' + formName + '" name="' + formName + '" encType="application/x-www-form-urlencoded" method="POST"></form>');
