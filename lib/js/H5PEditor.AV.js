...
(function ($) {
    ...
    H5PEditor.language.och5p = {};

    loadStrings();

    function loadStrings() {
        let opencastLanguage = {
            'label_course': 'Select a Course',
            'label_video_file': H5PEditor.t('core','selectFiletoUpload'),
            'label_video_flavor': H5PEditor.t('core','videoQuality'),
            'header_text': 'Opencast Videos',
        };
        let action = 'loadStrings';
        let languageStrings = opencastAjaxCallSync(action);
        if (languageStrings) {
            opencastLanguage = languageStrings;
        }

        H5PEditor.language.och5p.libraryStrings = opencastLanguage;
    }
    ...
    function getOpencastActionUrl(action) {
        var courseid = getUrlParamValues('course');
        var contentid = window.H5PEditor.contentId;

        var ocAjaxPath = H5PEditor.ajaxPath.replace('/mod/hvp', '/local/och5p');

        var OCActionUrl = ocAjaxPath + action;
        if (courseid) {
            OCActionUrl += '&courseid=' + courseid;
        }
        if (contentid) {
            OCActionUrl += '&contentid=' + contentid;
        }
        return OCActionUrl;
    }
    ...
    function getCourseVideosInsertDropdown() {
        ...
        if (action == 'courseVideos') {
            ...
            '<label for="h5p-oc-course-videos" class="h5peditor-label">' + H5PEditor.t('och5p', 'label_video_file') + '</label>' +
                '<select id="h5p-oc-course-videos" class="h5peditor-select h5p-oc-videos-selection">' + dropdownContent + '</select>' +
            '</div>';
        } else {
            dropdownDisplay = '<div class="field select">' +
                '<label for="h5p-oc-course-list" class="h5peditor-label">' + H5PEditor.t('och5p', 'label_course') + '</label>' +
        ...
        return  '<div class="h5p-oc-horizontal-separator"></div>' +
                '<div class="h5p-add-dialog-table">' +
                    '<div class="h5p-oc-dialog-box">' +
                        '<div class="h5p-oc-logo-header">' +
                            '<h3 class="h5p-oc-header-text">' + H5PEditor.t('och5p', 'header_text') + '</h3>' +
                        '</div>' +
                        '<div class="h5p-oc-video-wrapper">' +
                            dropdownDisplay +
                            '<div class="field select" style="display: none;">' +
                                '<label for="h5p-oc-video-quality" class="h5peditor-label">' + H5PEditor.t('och5p', 'label_video_flavor') + '</label>' +
