...
require_once($CFG->dirroot . '/local/och5p/och5p.php');
...
function local_och5p_extend_themes() {
    ...
    if (count($to_be_extended_themes)) {
        $extended_themes = och5p_extend_themes($to_be_extended_themes);
        if ($diffs = array_diff($to_be_extended_themes, $extended_themes)) {
            $theme_names = array();
            foreach ($diffs as $diff) {
                $theme_names[] = ucfirst(str_replace('_', ' ', $diff));
            }
            $error_message = sprintf(get_string('extended_error', 'local_och5p'), implode(', ', $theme_names));
    ...
    if (count($other_themes)) {
        $unextended_themes = och5p_unenxtend_themes($other_themes);
    }
